...
/**
 * Handler for the event of editing a handler.
 * 
 * @author Andrej Jurco
 *
 */
public class EditRecordHandler {
	...
	public boolean canExecute(MApplication application) {
		if (DataLoader.getPeriod(application) > 0 & DataLoader.getRecords(application).isSelectedOne())
			return true;
		return false;
	}
	...
	public void execute(Shell shell, MApplication application) {
		...
		RecordDialog rd = new RecordDialog(shell, (int) ti.getData("amount"), (Date) ti.getData("date"),
				(String) ti.getData("note"));
